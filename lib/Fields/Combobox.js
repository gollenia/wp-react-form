...
const Combobox = (props) => {
    const { onChange, options = [], hasEmptyOption, help, hint, disabled, placeholder = '', multiple, required = false, label = '', name = '', width = 6, className = '', customErrorMessage, tabIndex, id, } = props;
    const classes = [
        'ctx-form-field',
        'select',
        'input--width-' + width,
    ...
    return (_jsxs("div", { style: {
            gridColumn: `span ${width}`,
                    ...
                    setListSelect(-1);
                }, onClick: () => { }, tabIndex: tabIndex, id: id, placeholder: selection !== -1 ? options[selection] : placeholder, value: inputField, onChange: (event) => setInputField(event.target.value) }), _jsxs("ul", { children: [help !== '' && (_jsx("li", { className: listSelect === -1 ? 'selected' : '', onMouseDown: (event) => {
                            nullSelect();
