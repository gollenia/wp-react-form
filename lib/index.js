...
var GLOBAL_ALLOWED_ATTRIBUTES = Array.from(new Set(Object.values(ALLOWED_ATTRIBUTES_BY_TAG).flat()));
...
var sanitizeAttributes = (root) => {
  root.querySelectorAll("*").forEach((element) => {
    ...
    Array.from(element.attributes).forEach((attribute) => {
      ...
      if ((attribute.name === "href" || attribute.name === "src") && !isUrlSafe(attribute.value)) {
        element.removeAttribute(attribute.name);
      }
    });
...
var sanitize = (html2, allowedTags) => {
  const purifier = getPurifier();
  if (!purifier || typeof DOMParser === "undefined") {
    return html2;
  }
  const sanitizedHtml = purifier.sanitize(html2, {
...
var sanitizeHtml = (html2) => sanitize(html2, ALLOWED_BLOCK_TAGS);
var sanitizeInlineHtml = (html2) => sanitize(html2, ALLOWED_INLINE_TAGS);
